...
def precompute_targets(video_path, device, multires_size=MULTIRES_SIZE):
    ...
    # Store seg_targets on CPU as int16 to save ~700MB of VRAM (RTX 4060 compat).
    # Class indices are < 128 so int16 is lossless. Training loop must call
    # .long().to(device) before passing to the loss function.
    seg_targets_hard = torch.stack(seg_targets_hard).short().cpu()
